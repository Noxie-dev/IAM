...
describe('FAQPage Component - Core Functionality', () => {
  beforeEach(() => {
    jest.useFakeTimers();
  });
  ...
  test('renders correctly and displays initial data', () => {
    ...
    expect(screen.getByRole('button', { name: /filter by all category/i })).toHaveClass('bg-gradient-to-r');
  });
  ...
  test('filters FAQs by category (simplified)', async () => {
    ...
    // Check if filtering happened by verifying presence of a security-related FAQ
    await waitFor(() => {
        expect(screen.getByText(/is iam popia compliant/i)).toBeInTheDocument();
    });
  });

  test('modal functionality - basic open', async () => {
    const user = userEvent.setup();
    render(<FAQPage />);

    // Find and click on a FAQ card
    const cardQuestion = screen.getByText(/how do i get started with iam/i);
    const card = cardQuestion.closest('div');
    
    await user.click(card);

    // Check if modal content appears (even if not in a proper modal structure)
    await waitFor(() => {
